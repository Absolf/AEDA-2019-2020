...
Car::Car(std::string b, std::string m, float p): brand(b), model(m), price(p){}
...
FuelCar::FuelCar(string b, string m, float p, float t, float l_100km) : Car(b, m, p), tank(t), l_100km(l_100km) {
}
...
float HybridCar::range() const{
    return this->FuelCar::range() + this->ElectricCar::range();
}
