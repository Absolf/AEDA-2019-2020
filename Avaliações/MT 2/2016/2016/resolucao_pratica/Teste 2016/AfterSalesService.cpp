...
void AfterSalesService::enqueueArticles() {
    ...
    unsigned int auxQueueSize = toWrapQueueSize - toWrap.size();
    if (presentsTable.size() < auxQueueSize)
        auxQueueSize = presentsTable.size();
    while (counter < auxQueueSize) {
