...
{
    int infoVeiculo = Veiculo::info();
    o << "Combustivel: " << combustivel << endl;
    o << "Cilindrada: " << cilindrada << endl;
    return infoVeiculo+2;
}

int Motorizado::info() const { return Motorizado::info(cout);}
...
int Automovel::info() const {return Automovel::info(cout);}

int Camiao::info(ostream &o) const
{
    o << "Camiao" << endl;
    int infoMotorizado = Motorizado::info();
    o << "Carga maxima: " << carga_maxima << endl << endl;
    return infoMotorizado +1;
}

int Camiao::info() const {return Camiao::info(cout);}
...
int Bicicleta::info() const { return Bicicleta::info(cout);}
...
float Motorizado::calcImposto() const
{
    if(combustivel == "gasolina") {
        ...
        else if(ano <= 1995) return 148.37; else return 320.89;
    }
    else{
        if(cilindrada <= 1500)
            if(ano <= 1995) return 8.10; else return 14.56;
